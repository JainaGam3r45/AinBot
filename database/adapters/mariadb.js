...
class MariadbDatabaseAdapter extends SqlRecordAdapter {
    constructor(config) {
        super(config);
    ...
    async connect() {
        const mariadb = requireDriver("mariadb", ["mariadb"]);

        this.pool = mariadb.createPool(getConnectionOptions(this.config));
